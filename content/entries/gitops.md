---
title: "Gitops"
date: 2022-04-12T18:17:03+01:00
---
...
Das wichtigste Element ist ein `Source` das genutzt wird um die Quelle für ein Deployment zu definieren. Diese Quellen werden periodisch von Flux angefragt und auf Änderungen überprüft um flexibel auf diese Änderungen reagieren zu können.
...
Für einen ersten lokalen Test ist ein k3d Cluster völlig ausreichend. Die Installation erfolgt z.B. mit `brew install k3d`. Nach der Installation ist es einfach mit einem Kommando ein Cluster mit einer Portfreigabe zu Erstellen und zu starten: `k3d cluster create -p "8081:80@loadbalancer" --agents 2`.
3) Flux CLI
...
In dem Verzeichnis `./tekton-base` benutze ich dann eine Kustomization aus der Kustomize.io Welt um die eigentliche Installation vorzunehmen. Hier einmal beispielhaft zu sehen.

```yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
resources:
- https://storage.googleapis.com/tekton-releases/pipeline/previous/v0.34.1/release.yaml
- https://github.com/tektoncd/dashboard/releases/download/v0.25.0/tekton-dashboard-release.yaml
```

Auf diese Weise werden die Dateien und die beinhalteten Manifeste eingespielt und genutzt werden. Den aktuellen Zustand kann man während einer Installation über die Kommandozeile verfolgen: `kubectl get Kustomization -n flux-system`.
Mit diesen einfachen Bausteinen können nun weitere Installationen erfolgen und ein komplexes System aufgebaut werden.

## Fazit und Ausblick
Wir konnten mit wenig Aufwand mit Flux ein Installationsrezept nach GitOps Prinzipien aufbauen und beliebig ausrollen. Da Flux Pull-basiert arbeitet können wir mit diesem Tool sehr gut eine große Anzahl von Clustern mit der Installation versorgen.
Aus meiner Sicht ist es ein gutes Werkzeug für Personen mit einem Fokus auf die Infrastruktur, denn dort fügt es sich sehr gut in den Werkzeugkasten ein weil es mit wenig Overhead genutzt werden kann.